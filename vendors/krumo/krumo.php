...
This is a list of all the values from the <code><b><?php echo realpath($ini_file) ? realpath($ini_file) : $ini_file;?></b></code> INI file.
	...
	*/
	Public Static Function dump($data, $label = NULL) {
		...
		//
		/* 
		// Disabled this because I added $label as a second argument instead (sb 8/26/11)
		if (func_num_args() > 1) {
			$_ = func_get_args();
			foreach($_ as $d) {
				krumo::dump($d);
...
<div class="krumo-root">
	<ul class="krumo-node krumo-first">
		...
		<?php echo krumo::_dump($data, $label);?>
		
		<li class="krumo-footnote">		
			<?php if (@$d['file']) { ?>
			<span class="krumo-call">
				<code>
					<? @array_walk(debug_backtrace(),create_function('$a,$b','print "".basename($a[\'file\'])." (ln {$a[\'line\']}): {$a[\'function\']}();<br> ";')); ?>		
					<br>
					<?php echo $d['file']?> (ln <?php echo $d['line']?>)
				</code>
			</span>
			<?php } ?>
			&nbsp;
